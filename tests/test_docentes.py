...
class TestCriarDocente:
    ...
    def test_criar_docente_com_sucesso(self, client, usuario_teste, headers_autenticado):
    ...
    def test_criar_docente_email_duplicado(self, client, usuario_teste, headers_autenticado):
        """Deve retornar 400 se email já existe"""
        dados_docente_1 = {
            "nome": "Prof. 1",
            "email": "prof@example.com",
        }
        client.post("/api/v1/docentes/", json=dados_docente_1, headers=headers_autenticado)
...
class TestListarDocentes:
    ...
    def test_listar_docentes_do_usuario(self, client, usuario_teste, headers_autenticado):
        """Deve listar apenas docentes do usuário autenticado"""
        dados_docente = {
            "nome": "Prof. João",
            "email": "prof.joao@example.com",
        }
        client.post("/api/v1/docentes/", json=dados_docente, headers=headers_autenticado)
...
class TestObterDocente:
    ...
    def test_obter_docente_do_usuario(self, client, usuario_teste, headers_autenticado):
        ...
        response = client.get(f"/api/v1/docentes/{id_docente}", headers=headers_autenticado)
        
        assert response.status_code == 200
        assert response.json()["data"]["id_docente"] == id_docente

    def test_obter_docente_outro_usuario(self, client, usuario_teste, usuario_teste_2, headers_autenticado, headers_autenticado_usuario_2):
...
class TestAtualizarDocente:
    ...
    def test_atualizar_docente_completo(self, client, usuario_teste, headers_autenticado):
    ...
    def test_atualizar_docente_parcial(self, client, usuario_teste, headers_autenticado):
...
class TestDeletarDocente:
    ...
    def test_deletar_docente_com_sucesso(self, client, usuario_teste, headers_autenticado):
...
class TestObterDocentePorEmail:
    ...
    def test_obter_docente_por_email(self, client, usuario_teste, headers_autenticado):
        ...
        response = client.get(
            f"/api/v1/docentes/email/{email_docente}",
            headers=headers_autenticado
        )
